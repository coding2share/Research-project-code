...
twit2$hashtags <- as.factor(twit2$mention)
...
twit2rel$mention <- as.factor(twit2rel$mention)
...
quantile(twit2rel$RT, na.rm=T)
quantile(twit2rel$likes, na.rm=T)
quantile(twit2rel$replies, na.rm=T)
#thinspo vs. fitspo
...
quantile(twitfit$RT, na.rm=T)
quantile(twitfit$likes, na.rm=T)
quantile(twitfit$replies, na.rm=T)
sapply(twitthin, function(x) if(class(x)=="factor") freq(x, plot=F))
quantile(twitthin$RT, na.rm=T)
quantile(twitthin$likes, na.rm=T)
quantile(twitthin$replies, na.rm=T)
...
quantile(twituser$Followed, na.rm=T)
quantile(twituser$Followers, na.rm=T)
quantile(twituser$Tweets, na.rm=T)
quantile(twituser$Favorites, na.rm=T)

#thinspo and fitspo tweeter comparison for Table 2
...
quantile(twituserfit$Followed, na.rm=T)
quantile(twituserfit$Followers, na.rm=T)
quantile(twituserfit$Tweets, na.rm=T)
quantile(twituserfit$Favorites, na.rm=T)
sapply(twituserthin, function(x) if(class(x)=="factor") freq(x, plot=F))
quantile(twituserthin$Followed, na.rm=T)
quantile(twituserthin$Followers, na.rm=T)
quantile(twituserthin$Tweets, na.rm=T)
quantile(twituserthin$Favorites, na.rm=T)
...
is.na(topicthin) <- topicthin=="No"

topicfit <-subset(topic, thinfit=="fitspo")
is.na(topicfit) <- topicfit=="No"

w2 <- apply(topicthin, 2, table)
w3 <- apply(topicfit, 2, table)
final <- data.frame(Map(c, w2, w3))
final$names <- c("thinspo","fitspo")
...
library(reshape)
fitspodata <- rename(fitspodata, c(fitspo="tweets",pfitspo="ptweets"))
thinspodata <- final[thinvars]
thinspodata$thinfit <- "thinspo"
thinspodata <- rename(thinspodata, c(thinspo="tweets",pthinspo="ptweets"))
finallong <- rbind(fitspodata,thinspodata)
finallong <- subset(finallong, finallong$topic!="thinfit")

#Figure 1
library(ggplot2)
ggplot(data=finallong, aes(x=reorder(topic, ptweets), y=ptweets, fill=thinfit)) +
  ...
  scale_x_discrete(breaks=c("mentionloseweight", "mentionexercise", "mentiondisorder",
                   ...
                   labels=c("Losing weight", "Exercise", "Eating disorder",
                            'Wanting body','Eating less','Strength',
                            'Purging','Eating healthy','Medication',
                            'Binging',"None of these"))+
  labs(x="Topic in tweet", y="Percentage of tweets within\n#thinspo or #fitspo category", fill='')+
  scale_fill_manual(values=c('gray60','gray30'), labels = c("#fitspo", "#thinspo"))+ 
